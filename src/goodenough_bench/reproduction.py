...
def reproduction_payload(
    ...
    """Canonical reproduction payload for persisted provenance fingerprinting.
    ...
    This is the Phase 2 persisted-metadata fingerprint only. Case files, prompt
    templates, and scorer code are not hashed until a benchmark corpus exists.
    """
    ordered = sorted(
        planned_runs,
...
def verify_batch_reproduction(repository: Any, batch_id: str) -> BatchReproductionReport:
    """Recompute the persisted-metadata checksum and compare to the frozen batch record."""
