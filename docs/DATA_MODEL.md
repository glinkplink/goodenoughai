...
| **Key fields** | `batch_id`, `batch_purpose` (`diagnostic_pilot` or `stable_benchmark`), `suite_version_id` or candidate-manifest reference, `started_at`, `completed_at`, `status`, `reproduction_checksum` when frozen, provenance fields |
