...
**Phase 2 — Local benchmark foundation: in progress.** Tracked SQLite migrations (`0001_initial.sql`, `0002_batch_purpose.sql`, `0003_model_route_provenance.sql`, `0004_reproduction_checksum.sql`), a portable repository boundary for benchmark batches and planned runs, idempotent/resumable planned-run persistence, explicit `batch_purpose`, parent-batch/planned-run provenance enforcement, planned-run-bound collected-response validation, write-boundary revalidation and pricing-catalog resolution, material local-artifact/routed-provider identity persistence, strict batch lifecycle transitions (`planned → running → completed → frozen`), frozen-batch reproduction checksum metadata with CLI verification, statement-complete migration execution, an immutable filesystem artifact store with write-before-parse gating, resumable deterministic batch planning with a fake-provider test harness, and strict loaders for synthetic repository-controlled model profiles and dated pricing snapshots are implemented. The Python package, mostly placeholder CLI, strict Pydantic lifecycle boundaries, focused unit tests, and operational ignore rules remain in place. No runner execution, adapters, corpus, scoring implementation, or public application exists yet.
...
| Benchmark runner / database | Tracked SQLite migrations (`0001_initial.sql`, `0002_batch_purpose.sql`, `0003_model_route_provenance.sql`, `0004_reproduction_checksum.sql`), statement-complete migration runner, repository with batch-purpose and material model/route provenance persistence, parent-batch provenance enforcement, lifecycle transitions, and frozen `reproduction_checksum`; resumable deterministic batch planning (`RepositoryBatchPlanner`); immutable filesystem artifact store; no runner execution yet |
...
| Boundary tests | Focused `unittest` coverage for construction, serialization, provenance, migrations (including upgrade and statement parsing), repository idempotency/conflict rules, batch lifecycle transitions and reproduction checksum verification, resumable batch planning with fake-provider interruption/resume, profile/pricing loader validation, artifact store write/verify/corruption/conflict behavior, and CLI reproduce/placeholder behavior |
| Public web app | None |
...
5. Address PR #3 deferred adapter-boundary gaps (PB-001, PB-002) in runner/adapter slice or small follow-up PR when adapters begin.
