...
- Implemented strict model-profile and pricing-snapshot loaders (`profile_loaders.py`, `config/model_profiles/`, `config/pricing_snapshots/`) with Pydantic catalog boundaries, cross-reference validation, fixed surface-to-provider bindings (API surfaces plus `ollama_local` → `ollama`) that preserve routed/direct/local provenance, deterministic ordering, canonical JSON checksums, and synthetic fixtures explicitly marked as unverified placeholders; no migration, provider API calls, or verified prices
