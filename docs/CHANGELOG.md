...
- Implemented batch lifecycle transitions (`planned → running → completed → frozen`), migration `0004_reproduction_checksum.sql`, deterministic frozen-batch reproduction metadata/checksum helpers, and `goodenough-bench batch reproduce --verify-checksum` machine-readable CLI verification; scored `result_checksum` and full re-execution remain later work
...
- Made planned-run insertion conditional on the parent remaining `planned` and made batch reproduction report malformed persisted batch/plan data as machine-readable `invalid_data` rather than raising a traceback
