...
from goodenough_bench.migrations import apply_migrations, discover_migrations
...
class BatchLifecycleTests(unittest.TestCase):
    ...
    def test_planned_to_running_to_completed_to_frozen(self) -> None:
        ...
        self.assertEqual(completed.invalid_run_count, 0)
        self.assertEqual(completed.valid_for_scoring_count, 1)
    ...
    def test_freeze_requires_at_least_one_planned_run(self) -> None:
        self.repository.create_batch(planned_batch())
        self.repository.transition_batch("batch-001", BatchStatus.RUNNING, at=STARTED)
        self._complete_batch(invalid_run_count=0, valid_for_scoring_count=0)
    ...
    def test_stale_transition_cannot_overwrite_newer_state(self) -> None:
        ...
        def advance_to_frozen(batch_id: str) -> list[PlannedRun]:
            ...
            competing_repository.transition_batch(
                ...
                invalid_run_count=0, valid_for_scoring_count=1,
            )
    ...
    def test_freeze_rejects_constructed_run_bypass(self) -> None:
        batch = planned_batch()
        ...
        with self.assertRaisesRegex(BatchLifecycleError, "failed full boundary validation"):
            apply_batch_transition(
                completed,
                BatchStatus.FROZEN,
                planned_runs=[forged_run],
            )

        foreign_run = planned_run().model_copy(update={"batch_id": "batch-other"})
        with self.assertRaisesRegex(BatchLifecycleError, "belongs to batch"):
            apply_batch_transition(
                completed,
                BatchStatus.FROZEN,
                planned_runs=[foreign_run],
            )
