...
- **Use one deliberate review pass.** Complete local verification and self-review before requesting Codex/human review. Batch feedback into one update where practical, then request a final re-review of the resulting head rather than one review cycle per comment or exploratory push. Follow [`docs/GIT_WORKFLOW.md`](docs/GIT_WORKFLOW.md#pr-readiness-and-review-protocol) and use [the PR template](.github/PULL_REQUEST_TEMPLATE.md).
