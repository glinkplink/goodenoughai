...
| [src/goodenough_bench/migrations/](src/goodenough_bench/migrations/) | Tracked SQL migrations (`0001_initial.sql`–`0004_reproduction_checksum.sql`) and statement-complete, checksum-verified migration runner; `0004` adds nullable `reproduction_checksum` and conservatively reclassifies legacy `frozen` batches on upgrade | **Implemented for current persistence schema** |
